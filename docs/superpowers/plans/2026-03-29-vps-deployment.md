# FlowForge VPS Deployment Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Deploy FlowForge to a Hostinger KVM 2 VPS with `stackless.cloud` (frontend) and `api.stackless.cloud` (backend) behind Nginx + Let's Encrypt SSL using a single automated bash script.
...
services:
  mongodb:
    ...
    environment:
      ...
      MONGO_INITDB_DATABASE: ${MONGODB_DB_NAME:-flowforge}
  ...
  rustfs:
    ...
    container_name: flowforge-rustfs
    restart: unless-stopped
    environment:
  ...
  backend:
    ...
    env_file:
      - .env.production
    environment:
      MONGODB_URI: mongodb://${MONGO_INITDB_ROOT_USERNAME}:${MONGO_INITDB_ROOT_PASSWORD}@mongodb:27017/${MONGODB_DB_NAME:-flowforge}?authSource=admin
  ...
  celery-worker:
    build:
      ...
      dockerfile: Dockerfile
    image: flowforge-backend:latest
    container_name: flowforge-celery-worker
    restart: unless-stopped
  ...
  celery-beat:
    build:
      ...
      dockerfile: Dockerfile
    image: flowforge-backend:latest
    container_name: flowforge-celery-beat
    restart: unless-stopped
  ...
  frontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile
    image: flowforge-frontend:latest
...
SMTP_USE_TLS=true
EMAIL_FROM=FlowForge <noreply@stackless.cloud>
...
git clone https://github.com/YOUR_USERNAME/YOUR_REPO.git /opt/flowforge
cd /opt/flowforge
```

Option B — upload from local machine (run on your Mac):
```bash
scp -r /Users/snowden/project/no_code_builder/ root@<your-vps-ip>:/opt/flowforge
```
...
```
NAME                        STATUS
flowforge-mongodb           Up (healthy)
flowforge-redis             Up (healthy)
flowforge-rustfs            Up (healthy)
flowforge-backend           Up (healthy)
flowforge-celery-worker     Up
flowforge-celery-beat       Up
flowforge-frontend          Up (healthy)
```
...
# Backup MongoDB
docker exec flowforge-mongodb mongodump \
  --username admin \
